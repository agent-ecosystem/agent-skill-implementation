...
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/activation-loading-scope/transcript_full.jsonl`; session d54b7584-8fbb-4163-98b7-43188d071aa0):
  - model's tool call targets the skill's own path (event 5, line 3)
  - body canary arrived in the tool result (event 6, line 4)
...
- **Vehicle**: model-pull
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/eager-link-resolution/transcript_full.jsonl`; session 6aa92e0d-57aa-4c99-8076-ea0b8984626d):
  - skill body loaded (event 6, line 4)
  - references/setup-guide.md arrived only via the model's own read (event 14, line 9)
  - references/troubleshooting.md arrived only via the model's own read (event 17, line 11)
  - references/unlinked-data.md arrived only via the model's own read (event 20, line 13)
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/directory-naming-divergence/transcript_full.jsonl`; session 397d6c9c-a0b3-43a7-9cd5-89798a6edfd1):
  - resources/ file content arrived via the model's own read (event 25, line 17)
...
- **Note**: final answer: Based on the instructions in [`SKILL.md`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-2668306625/project/.agents/skills/probe-nonstandard-dirs/SKILL.md), here is the report on directory handling for `probe-nonstandard-dirs`:  ---  ### 1. Directory Awareness The platform's initial skill metadata prompt provided only the path to `SKILL.md`. Reading [`SKILL.md`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-2668306625/project/.agents/skills/probe-nonstandard-dirs/SKILL.md) described the presence of three directories, which were confirmed via direct filesystem listing:  * `evals/` * `resources/` * `templates/`  ---  ### 2. Content Visibility * **Initial Context (Pre-Tool Execution)**: Subdirectory file listings and file contents are **not** pre-loaded, pre-injected, or automatically visible in the initial context window upon skill activation. * **Via File Tools**: With standard filesystem tools (`list_dir` / `view_file`), all subdirectories and their contents are fully accessible:   * [`evals/`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-2668306625/project/.agents/skills/probe-nonstandard-dirs/evals) contains `evals.json`   * […
- **Fallback behavior**: Not exercised: automated single-session runs do not probe recovery paths (no follow-up prompting). Treat as untested rather than absent.
...
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/path-resolution-base/run-01/transcript_full.jsonl`; session a03fe60b-4b56-403e-a3f2-d5eba76d0e22):
  - bare SKILL.md-relative path attempt did not deliver content (event 10, line 6)
...
- **Vehicle**: model-pull
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/cross-skill-resource-shadowing/transcript_full.jsonl`; session d45779fa-5420-44b1-98f6-fbb1d55d670f):
  - activated skill's own API.md content arrived first (event 10, line 7)
  - sibling skill's API.md also read (event 13, line 9)
...
- **Note**: final answer: I have activated the **probe-shadow-alpha** skill and completed its instructions:  ### 1. Contents of `probe-shadow-alpha` API Reference  File: [`references/API.md`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-2306791246/project/.agents/skills/probe-shadow-alpha/references/API.md)  ```markdown # API Reference (Alpha)  Shadow-alpha canary phrase: **STORK-CORAL-4471**  This file belongs to the `probe-shadow-alpha` skill. If you see this content when reading from the `probe-shadow-beta` skill's context, the platform is resolving resource paths ambiguously across skills.  ## Alpha Endpoints  - `GET /alpha/status` - Alpha health check - `POST /alpha/process` - Alpha processing ```  ---  ### 2. Canary Phrase Verification  * **Canary Phrase Identified**: **STORK-CORAL-4471** * **Result**: Confirmed as the expected canary phrase for the `probe-shadow-alpha` skill.  ---  ### 3. Cross-Skill Access Test (`probe-shadow-beta`)  Both skills are available in the workspace. Reading the corresponding reference file from the `probe-shadow-beta` skill was successful:  File: [`references/API.md`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-2306791246/project/.ag…
- **Fallback behavior**: Not exercised: automated single-session runs do not probe recovery paths (no follow-up prompting). Treat as untested rather than absent.
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/path-traversal-boundary/transcript_full.jsonl`; session aed853d8-96b9-4520-a845-b177e0b62cd3):
  - sibling skill's body canary arrived in a tool result (event 10, line 7)
  - first ../ path attempt (event 9, line 6)
- **Note**: relative ../ path forms were used
- **Note**: final answer: ### Path Traversal Probe Results  I have executed the path traversal probe tests as specified in [probe-traversal SKILL.md](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-781866117/project/.agents/skills/probe-traversal/SKILL.md). Here are the detailed results for each attempt:  ---  #### 1. Sibling Skill (`../probe-loading/SKILL.md`) * **Path Attempted:** `/var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-781866117/project/.agents/skills/probe-traversal/../probe-loading/SKILL.md` * **Did the platform allow the read?** Yes. * **Did you get the file contents?** Yes, successfully retrieved the complete content of [probe-loading/SKILL.md](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-781866117/project/.agents/skills/probe-loading/SKILL.md) (52 lines). * **Did the platform show an error or warning?** No. * **Did the platform silently return nothing?** No.  ---  #### 2. Parent Directory (`../README.md` & listing `../`) * **Path Attempted:** `/var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-781866117/project/.agents/skills/probe-traversal/../` * **Did the platform allow the read?** Yes. * **Did you get the file contents / listing?**    * …
- **Fallback behavior**: Not exercised: automated single-session runs do not probe recovery paths (no follow-up prompting). Treat as untested rather than absent.
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/resource-nesting-depth/transcript_full.jsonl`; session f653b813-2ceb-407c-a2fd-5ee3aba42610):
  - depth-1 file references/overview.md content arrived (event 11, line 7)
  - depth-2 file references/api/endpoints.md content arrived (event 14, line 9)
  - depth-3 file references/api/v2/migration-guide.md content arrived (event 17, line 11)
  - depth-3 file references/guides/advanced/performance-tuning.md content arrived (event 20, line 13)
  - depth-5 file references/api/v2/history/deprecated/removed-endpoints.md content arrived (event 23, line 15)
- **Fallback behavior**: Not exercised: automated single-session runs do not probe recovery paths (no follow-up prompting). Treat as untested rather than absent.
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/batch4-2026-08-01/antigravity/bundled-script-execution/transcript_full.jsonl`; session add1006d-75d2-45ca-b2b9-36718c50e155):
  - tool call references the bundled script (event 9, line 6)
  - runtime-assembled output phrase arrived in a tool result (event 10, line 7)
- **Note**: final answer: probe-script-execution activated. Canary: **REDSHANK-SYENITE-8807**  The bundled script [`emit-canary.sh`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-1630735590/project/.agents/skills/probe-script-execution/scripts/emit-canary.sh) printed the following output:  `GODWIT-BORNITE-5148`  ### Summary of Work - Activated the `probe-script-execution` skill by reading [`SKILL.md`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-1630735590/project/.agents/skills/probe-script-execution/SKILL.md). - Executed the bundled script `scripts/emit-canary.sh` using bash. - Captured and reported the exact runtime output.
- **Fallback behavior**: Not exercised: automated single-session runs do not probe recovery paths (no follow-up prompting). Treat as untested rather than absent.
...
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/discovery-listing-fields/transcript_full.jsonl`; session 69e4f6e8-e94c-40bd-b1b7-5802a8c8a6d8):
  - tool-free catalog echo names the installed skills (event 4, line 3)
  - description text surfaced at discovery (event 4, line 3)
  - SKILL.md file path surfaced at discovery (event 4, line 3)
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/frontmatter-handling/transcript_full.jsonl`; session b18d9993-9187-4ea0-811a-eb90d74e7c06):
  - raw file read delivered frontmatter and body together (event 6, line 4)
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/content-wrapping-format/transcript_full.jsonl`; session f6737bfc-ff80-4285-a544-a2bbc489d916):
  - body arrived as a file-read tool result (event 6, line 4)
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/reactivation-deduplication/transcript_full.jsonl`; session 10ee664a-1986-4561-8419-adf99d257116):
  - first read of skill body (event 6, line 4)
  - second read of skill body (event 15, line 10)
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/reactivation-freshness/transcript_full.jsonl`; session ce20d585-23c2-4be8-80e6-6a5f325959b1):
  - post-edit canary reached the model on reactivation (event 15, line 10)
- **Fallback behavior**: Not exercised: automated single-session runs do not probe recovery paths (no follow-up prompting). Treat as untested rather than absent.
...
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/batch4-2026-08-01/antigravity/allowed-tools-behavior/session-2/transcript_full.jsonl`; session 2bace1c6-a11b-4e55-b43b-9b7cd0f30c18,588b196b-17af-48af-8138-dc54dd95fc29):
  - with-field session: runtime-assembled phrase arrived in a tool result (session 1, event 10, line 7)
  - control session: runtime-assembled phrase arrived in a tool result (session 2, event 10, line 7)
...
- **Note**: with-field final answer: probe-allowed-tools activated. Canary: **CURLEW-SCHIST-4419**  ### 1. `allowed-tools` Visibility Yes, the frontmatter of [SKILL.md](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-2955770399/project/.agents/skills/probe-allowed-tools/SKILL.md#L4) specifies: ```yaml allowed-tools: Bash(printf:*) Read ```  ### 2. Command Execution & Output Ran command: `printf 'GROUSE-%s-9017\n' 'MICA'`  **Output:** ``` GROUSE-MICA-9017 ```  ### 3. Permission Prompt / Approval Step Observation The command executed immediately and returned exit code `0` without any permission prompts, errors, or manual approval steps required.
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/cross-skill-invocation/transcript_full.jsonl`; session 04feff94-4f96-4d0e-8da0-f3d881dc11e3):
  - invoke-alpha body loaded (chain entry) (event 6, line 4)
  - invoke-beta's body canary loaded after invoke-alpha's instruction (event 10, line 7)
- **Note**: invoke-gamma deliberately not installed; the chain tail beyond beta is out of scope here
- **Note**: final answer: invoke-alpha activated. Canary: **IBIS-RUST-3310**  invoke-beta activated. Canary: **TERN-MOSS-6647**  invoke-beta complete. invoke-gamma said: `invoke-gamma` is not available (skill path not found).  invoke-alpha complete. Chain result: - [`invoke-alpha`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-1956458607/project/.agents/skills/invoke-alpha/SKILL.md) executed successfully with canary **IBIS-RUST-3310**. - [`invoke-beta`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-1956458607/project/.agents/skills/invoke-beta/SKILL.md) executed successfully with canary **TERN-MOSS-6647**. - `invoke-gamma` could not be activated because the skill is missing from `.agents/skills/`.  ---  ### Summary 1. Read instructions for [`invoke-alpha/SKILL.md`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-1956458607/project/.agents/skills/invoke-alpha/SKILL.md) and reported canary **IBIS-RUST-3310**. 2. Activated [`invoke-beta/SKILL.md`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-1956458607/project/.agents/skills/invoke-beta/SKILL.md) and reported canary **TERN-MOSS-6647**. 3. Checked for `invoke-gamma` and confirmed it is not p…
- **Fallback behavior**: Not exercised: automated single-session runs do not probe recovery paths (no follow-up prompting). Treat as untested rather than absent.
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/invocation-depth-limit/transcript_full.jsonl`; session 9c03fd1d-44ca-4d89-8a50-7d66517db7a8):
  - invoke-alpha body loaded (event 6, line 4)
  - invoke-beta body loaded (event 10, line 7)
  - invoke-gamma body loaded (event 13, line 9)
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/invocation-language-sensitivity/transcript_full.jsonl`; session 6be7f643-abec-4331-9aa8-e6376b9e72aa):
  - invoke-alpha body loaded (event 6, line 4)
  - invoke-beta body loaded (event 10, line 7)
  - invoke-gamma body loaded (event 13, line 9)
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/informal-dependency-resolution/transcript_full.jsonl`; session 53d1b8d0-945d-4a68-8f74-3cefaa967d6e):
  - invoke-alpha body loaded (chain entry) (event 6, line 4)
  - invoke-beta's body canary loaded after invoke-alpha's instruction (event 10, line 7)
- **Note**: invoke-gamma deliberately not installed; the chain tail beyond beta is out of scope here
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/missing-dependency-behavior/transcript_full.jsonl`; session c17ac036-2b37-469a-93c3-02f36b2c4dc5):
  - probe-missing-dep body loaded (event 6, line 4)
  - model surfaced the missing dependency in its own text without attempting it (event 8, line 6)
...
- **Vehicle**: model-pull
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/nonstandard-dependency-fields/transcript_full.jsonl`; session aceb217c-3203-485d-92bd-dcc9489204da):
  - probe-nonstandard-fields body loaded (event 6, line 4)
- **Note**: final answer: I have activated the **probe-nonstandard-fields** skill by viewing its [`SKILL.md`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-1848308436/project/.agents/skills/probe-nonstandard-fields/SKILL.md) file and reviewing the nonstandard frontmatter metadata. Here is the probe report:  ---  ### 1. Field Visibility Yes, the nonstandard frontmatter fields are fully visible when reading [`SKILL.md`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-1848308436/project/.agents/skills/probe-nonstandard-fields/SKILL.md#L1-L9). Their exact parsed values are:  - **`requires`**: `probe-loading` - **`depends-on`**:   - `probe-shadow-alpha`   - `probe-shadow-beta` - **`priority`**: `high`  ---  ### 2. Platform Behavior The platform **silently ignored** these nonstandard fields: - **No Dependency Auto-Loading**: The platform did not attempt to automatically load or inject `probe-loading`, `probe-shadow-alpha`, or `probe-shadow-beta` upon activating `probe-nonstandard-fields`. - **No Priority Re-ordering**: The platform did not modify skill ordering or alter execution logic based on `priority: high`.  ---  ### 3. Validation The platform **accepted the nonstandard fiel…
- **Fallback behavior**: Not exercised: automated single-session runs do not probe recovery paths (no follow-up prompting). Treat as untested rather than absent.
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/cross-scope-dependency/session-2/transcript_full.jsonl`; session 69fb8392-0fd9-4a17-8dee-a794e9b12f60,0e60e8ab-5132-49db-9358-ead437280584):
  - user-scope dependency's body canary pulled during project-skill session (event 10, line 7)
  - model surfaced the missing dependency in its own text without attempting it (session 2, event 17, line 12)
- **Note**: with-dependency final answer: probe-cross-scope activated. Canary: **CRANE-STEEL-1163**  ### 1. Installation Scope `probe-cross-scope` is a **project-level skill** located at [SKILL.md](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-2386038488/project/.agents/skills/probe-cross-scope/SKILL.md).  ### 2. Cross-Scope Resolution of `probe-loading` - **Found**: Yes, the platform successfully located `probe-loading`. - **Scope**: `probe-loading` was resolved at a **different scope (user-level)** at [SKILL.md](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-2386038488/home/agy/.gemini/config/skills/probe-loading/SKILL.md). - **Activation Status**: Successfully loaded and read the `probe-loading` instructions.  probe-cross-scope complete. Cross-scope resolution result: Project-level skill successfully identified and activated the user-level skill `probe-loading`.
...
- **Note**: without-dependency final answer: probe-cross-scope activated. Canary: **CRANE-STEEL-1163**  ### Installation Scope Report * **Scope**: Project-level skill * **Location**: [/var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-2290681380/project/.agents/skills/probe-cross-scope/SKILL.md](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-2290681380/project/.agents/skills/probe-cross-scope/SKILL.md)  ### Activation of `probe-loading` Skill * **Found**: No * **Failure Mode**: The skill `probe-loading` is not present in the available skills list in the environment context, nor does it exist on the local file system at user or project levels. Consequently, it could not be resolved or loaded.  probe-cross-scope complete. Cross-scope resolution result: The cross-scope dependency `probe-loading` could not be resolved because it is missing/not installed in the user or project environment.
- **Fallback behavior**: Not exercised: automated single-session runs do not probe recovery paths (no follow-up prompting). Treat as untested rather than absent.
...
- **Vehicle**: model-pull
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/cross-client-directory-interop/transcript_full.jsonl`; session 624829ae-0182-4fd6-8f1f-c2d8af263320):
  - tool-free listing answer names probe-interop (event 4, line 3)
  - body canary loaded on activation (event 11, line 8)
...
- **Note**: final answer: probe-interop activated. Canary: **SNIPE-OCHRE-2217**  **Discovery**: Yes, the `probe-interop` skill appeared in my available skills catalog at the path [`/var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-4246582053/project/.agents/skills/probe-interop/SKILL.md`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-4246582053/project/.agents/skills/probe-interop/SKILL.md).
- **Fallback behavior**: Not exercised: automated single-session runs do not probe recovery paths (no follow-up prompting). Treat as untested rather than absent.
...
- **Vehicle**: model-pull
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/recursive-root-discovery/transcript_full.jsonl`; session ec557376-b21c-4eeb-b5fd-97b927ecd2b2):
  - body loaded via file access, not catalog activation (event 26, line 18)
...
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/nested-skill-discovery/transcript_full.jsonl`; session 1a0313a0-d34f-4f63-b747-2b544520ca7a):
  - model's tool-free listing answer names only the outer skill (event 4, line 3)
...
- **Vehicle**: model-pull
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/name-collision-precedence/transcript_full.jsonl`; session a89c8ad1-e9b6-4e82-8511-a93180f78940):
  - project variant's canary loaded; user variant's never appeared (event 6, line 4)
...
- **Vehicle**: model-pull
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/missing-description-handling/transcript_full.jsonl`; session b31b4c2c-89e3-4d94-a07d-b244cdbfc4a1):
  - tool-free listing answer names probe-no-description (event 4, line 3)
  - body canary loaded on activation (event 11, line 8)
...
- **Vehicle**: model-pull
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/batch4-2026-08-01/antigravity/invalid-name-tolerance/transcript_full.jsonl`; session e936836d-f4b9-483e-a6a9-ff6fed3094e8):
  - tool-free listing answer names probe-Upper-Case (event 4, line 3)
  - tool-free listing answer names probe--double-hyphen (event 4, line 3)
  - tool-free listing answer names probe-overlong-name-padded-well-past-the-spec-sixty-four-character-limit (event 4, line 3)
...
- **Vehicle**: model-pull
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/name-directory-mismatch/transcript_full.jsonl`; session 65de6f10-1a8a-443e-a0ab-97a65dee00e6):
  - tool-free listing answer names probe-name-mismatch (event 4, line 3)
...
- **Vehicle**: model-pull
- **Confidence**: transcript-direct
- **Evidence** (transcript: `results/sandboxed-2026-08-01/antigravity/metadata-value-edge-cases/transcript_full.jsonl`; session 5763c381-2a3d-40a9-85dd-606e7685e414):
  - body canary loaded (event 6, line 4)
  - frontmatter-only marker '!!null' in a tool result (event 6, line 4)
- **Note**: edge-case metadata values reached the model only via its own raw file read
- **Note**: final answer: I have activated and evaluated the [`probe-metadata-values`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-644509556/project/.agents/skills/probe-metadata-values/SKILL.md) skill.  **Canary Phrase**: **THRUSH-FLINT-8294**  ---  ### Report  1. **Load Success**:     - **Yes**, the skill loaded successfully. The file [`SKILL.md`](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-644509556/project/.agents/skills/probe-metadata-values/SKILL.md) was accessed and read without rejection or parsing errors.  2. **Metadata Visibility**:    - The `metadata` field in the frontmatter contains the following key-value pairs:      - `normal`: `"baseline"` (Control string)      - `empty-double`: `""` (Empty string)      - `empty-single`: `''` (Empty string)      - `explicit-null`: `null` (Literal YAML null)      - `tilde-null`: `~` (YAML tilde shorthand for null)      - `none-null`: `None` (Unquoted literal `None`)      - `tagged-null`: `!!null null` (Explicit YAML tag `!!null` with value `null`)  3. **Key Preservation**:    - **All 7 keys are present**. None of the keys were stripped or omitted during indexing or file access.  4. **Platform Behavior**:    - The platfo…
...
- **Vehicle**: model-pull
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/batch4-2026-08-01/antigravity/oversize-description-handling/transcript_full.jsonl`; session e686cb8d-5531-4668-a75c-19973ed3b9e3):
  - tool-free listing answer names probe-long-description (event 4, line 3)
  - body canary loaded on activation (event 11, line 8)
...
- **Vehicle**: model-pull
- **Confidence**: behavioral-inference
- **Evidence** (transcript: `results/batch4-2026-08-01/antigravity/oversize-compatibility-handling/transcript_full.jsonl`; session 63c70e47-0f8e-411f-87de-1b0856f8492f):
  - tool-free listing answer names probe-long-compatibility (event 4, line 3)
  - body canary loaded on activation (event 11, line 8)
- **Note**: the compatibility value's tail marker is visible only via the model's own raw file read
- **Note**: final answer: probe-long-compatibility activated. Canary: **KESTREL-BAUXITE-6690**  **Field visibility**: No, the `compatibility` field value for this skill is not visible anywhere in the initial prompt or context outside of reading [SKILL.md](file:///var/folders/39/30d_262j423_n546m9g8_hc80000gn/T/skillxp-3260817820/project/.agents/skills/probe-long-compatibility/SKILL.md) directly. The system prompt's `Available skills` listing only surfaces the skill's name, file path, and `description` field.
